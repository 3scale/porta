...
class ApiDocs::Service < ApplicationRecord
  ...
  scope :permitted_for, ->(user = nil) { user ? where.has { (service_id == nil) | service_id.in(user.accessible_services.select(:id)) } : where.not(id: nil) }
  ...
  # [Deprecated][TODO] remove with service_id
  def service_id=(new_service_id)
    self.owner_id = self.owner_type = nil
    super(new_service_id)
  end
  ...
  def api_product_production_public_base_url
    (service&.proxy || owner.try(:proxy))&.endpoint
  end
