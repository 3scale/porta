...
class Proxy < ApplicationRecord
  ...
  def async_deploy(user)
    ProviderProxyDeploymentV1Service.async_deploy(user, self)
  end
  ...
  def sandbox_deployed?
    ...
    proxy_log.created_at > self.created_at && proxy_log.status == ProviderProxyDeploymentV1Service::SUCCESS_MESSAGE
  end
  ...
  def deploy_v1
    deployment = ProviderProxyDeploymentV1Service.new(provider)
