...
module Account::CreditCard # rubocop:disable Metrics/ModuleLength(RuboCop)
  ...
  def unstore_credit_card!
    begin
      ...
      # TODO: An exception would fail the operation but should we notify user on a failure response?
  ...
  def unstore_credit_card_on_destroy!
    ...
    notify_gateway_response(response, "unstore [auth: #{credit_card_auth_code}]")
  rescue => error
    Accounts::CreditCardUnstoreFailedEvent.create(self, error.message)
  end
  ...
  def notify_gateway_response(response, action)
    if response&.failure?
      Accounts::CreditCardUnstoreFailedEvent.create(self, response.inspect)
    end
  end
end
