...
interface Props {
  ...
  liquidReferenceHref: string;
  quickstartsHref: string | null;
  saas?: boolean;
  signOutHref: string;
}
...
const CUSTOMER_PORTAL_PATH = '//access.redhat.com/products/red-hat-3scale'
const WHATS_NEW_PATH = '//access.redhat.com/articles/3107441#newfeaturesenhancements'
...
const Masthead: FunctionComponent<Props> = ({
  ...
  const documentationItems = [
    { title: 'Customer Portal', href: CUSTOMER_PORTAL_PATH, icon: <ExternalLinkAltIcon />, target: '_blank' },
    { title: '3scale API Docs', href: apiDocsHref, icon: <PuzzlePieceIcon /> },
    { title: 'Liquid Reference', href: liquidReferenceHref, icon: <CodeIcon /> },
    saas ? { title: "What's new?", href: WHATS_NEW_PATH, icon: <LeafIcon />, target: '_blank' } : undefined,
    quickstartsHref ? { title: 'Quick starts', href: quickstartsHref, icon: <BookIcon /> } : undefined
  ].filter(Boolean)
    // @ts-expect-error Fucking bullshit
    .map(({ title, href, icon, target }: { title: string; href: string; target: string; icon: string }) => (
  ...
  const sessionItems = [
    ...
    <DropdownItem key="sign-out" href={signOutHref}>
      <Icon isInline className="header-context-selector__item-icon"><TimesIcon /></Icon>Sign Out
    </DropdownItem>
  ]
