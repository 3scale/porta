...
type Constraints = Partial<Record<FieldName, Partial<Record<ValidatorName, ValidatorOption>>>>;
type FieldState = Partial<Record<FieldName, string>>;
...
const useFormState = (): IUseFormState => {
  ...
  const onFieldBlur = (fieldName: FieldName) => (event: React.ChangeEvent<HTMLInputElement>) => {
