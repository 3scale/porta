...
And "there should be {int} application cancelled event" do |n|
...
And "there should be {int} valid service contract cancelled event" do |count|
  check_events_validity!(type: ServiceContracts::ServiceContractCancellationEvent, count: count)
end

And "there should be {int} valid service contract created event" do |count|
  check_events_validity!(type: ServiceContracts::ServiceContractCreatedEvent, count: count)
end

And "there should be {int} valid account created event" do |count|
  check_events_validity!(type: Accounts::AccountCreatedEvent, count: count)
end

And "there should be {int} valid account deleted event" do |count|
  check_events_validity!(type: Accounts::AccountDeletedEvent, count: count)
end

And "there should be {int} valid application created event" do |count|
  check_events_validity!(type: Applications::ApplicationCreatedEvent, count: count)
end

And "there should be {int} valid cinstance cancellation event" do |count|
