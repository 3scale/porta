...
Given('the provider has {word} portal Permissions-Policy {string}') do |portal, policy_value|
...
Then('the {word} portal should have Permissions-Policy header {string}') do |portal, expected_value|
...
Then('the {word} portal should not have Permissions-Policy header') do |portal|
  setting = @provider.account_settings.find_by(type: "AccountSetting::PermissionsPolicyHeader#{portal.capitalize}")
