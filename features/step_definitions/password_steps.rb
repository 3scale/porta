...
Given "no user exists with an email of {string}" do |email|
...
When "I follow the link found in the password reset email send to {string}" do |email|
  visit_url_in_email(email, /Lost password recovery/)
end

When "I follow the link found in the provider password reset email send to {string}" do |email|
