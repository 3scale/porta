...
Given "{provider} logs in" do |provider|
  ...
  try_provider_login(username, 'Super$ecr3t!!!!!')
...
Given /^I am logged in as (provider )?"([^\"]*)"$/ do |provider,username|
  if provider
    ...
    try_buyer_login_internal(username, 'Super$ecr3t!!!!!')
  end
  assert user_is_logged_in(username)
end
...
When "{buyer} logs in" do |buyer|
  ...
  try_buyer_login_internal(user.username, user.password || 'Super$ecr3t!!!!!')
...
When /^I fill in the "([^"]*)" login data$/ do |username|
  ...
  fill_in('Password', :with => "Super$ecr3t!!!!!")
...
When "{user} logs in" do |user|
  log_out
  try_provider_login(user.username, 'Super$ecr3t!!!!!')
  assert user_is_logged_in(user.username)
end
