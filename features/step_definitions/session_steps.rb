...
Given "{provider} logs in" do |provider|
  ...
  try_provider_login(username, 'Supersecret123+!')
...
Given /^I am logged in as (provider )?"([^\"]*)"$/ do |provider,username|
  if provider
    ...
    try_buyer_login_internal(username, 'Supersecret123+!')
  end
  assert user_is_logged_in(username)
end
...
When "{buyer} logs in" do |buyer|
  ...
  try_buyer_login_internal(user.username, user.password || 'Supersecret123+!')
...
When "{user} logs in" do |user|
  log_out
  try_provider_login(user.username, 'Supersecret123+!')
  assert user_is_logged_in(user.username)
end
