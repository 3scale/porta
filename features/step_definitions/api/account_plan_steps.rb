...
And "provider has {string} hidden" do |field|
  current_account.settings.toggle!(field)
end

And "provider has {plan_type} plans hidden from the ui" do |type_of_plan|
  current_account.settings.update!("#{type_of_plan}_plans_ui_visible": false)
end
