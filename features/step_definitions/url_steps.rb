...
Given "current domain is the {word} domain of {provider}" do |level, provider|
  ...
  set_current_domain(level == 'admin' ? provider.external_admin_domain : Account.master.external_domain)
  @provider = provider
end

Then /^the current domain should be ([^\s]+)$/ do |domain|
  assert_current_domain domain
end
...
Then "the current domain should be the admin domain of {provider}" do |provider|
  ...
  assert_equal provider.external_admin_domain, uri.host
end
