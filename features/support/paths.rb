...
module PathsHelper
  ...
  class PathFinder
    ...
    def path_to(page_name, *args)
      ...
      when /^the "([^"]*)" topic page$/
        forum_topic_path(Topic.find_by!(title: $1))
      ...
      when 'the edit active docs page'
        edit_admin_api_docs_service_path(provider_api_docs_service_first)
      when 'the preview active docs page'
        preview_admin_api_docs_service_path(provider_api_docs_service_first)

      when 'the service active docs page'
        admin_service_api_docs_path(provider_default_service)
      when 'the new active docs page for a service'
        new_admin_service_api_doc_path(provider_default_service)
      when 'the preview active docs page for a service'
        service = provider_default_service
      ...
      when /^the API alerts page of service "(.+?)" of provider "(.+?)"$/
        provider = Account.providers.find_by! org_name: $2
        service = provider.services.find_by! name: $1
        admin_service_alerts_path(service)

      when /^the (edit|settings) page for service "([^"]+)" of provider "(.+?)"$/
        provider = Account.providers.find_by! org_name: $3
        service = provider.services.find_by! name: $2
      ...
      when /^the provider side edit page for application "([^"]*)" of buyer "([^"]*)"$/
        application = Account.find_by!(org_name: $2).bought_cinstances.find_by!(name: $1)
        edit_admin_service_application_path(application.service, application)
      ...
      when /^the provider side "([^"]*)" topic page$/
        admin_forum_topic_path(Topic.find_by!(title: $1))
      when /^the provider side edit "([^"]*)" topic page$/
        edit_admin_forum_topic_path(Topic.find_by!(title: $1))
      ...
      when /^the edit page of the html block "([^"]*)"$/
        html_block = HtmlBlock.find_by!(name: $1)
