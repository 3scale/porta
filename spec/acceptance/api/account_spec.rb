...
resource 'Account' do
  ...
  before { FieldsDefinition.create_defaults!(Account.master) }
  ...
  #
  #     let(:username) { resource.users.order(created_at: :asc).first!.username }
  #   end
  #
  ...
  json(:resource) do
    ...
    # let(:resource) do
    #   FactoryBot.create(:simple_buyer) #.decorate
    # end
    ...
    context 'buyer account' do
      it do
        # binding.pry
        # subject
        should have_properties('id', 'org_name', 'state', 'credit_card_stored').from(account)
        should have_properties('created_at', 'updated_at')
        should have_links('self', 'users')
        # should have_properties('admin_user_display_name').from(account)
      end
    ...
    context 'provider account' do
      let(:resource) do
        FactoryBot.build(:simple_provider, support_email: 'support@email.com',
                          finance_support_email: 'finance@email.com', site_access_code: 'access-code').decorate
      end

      it { should have_properties(expected_provider_fields).from(resource) }
      # it { should have_properties('admin_user_display_name').from(resource) }
    end
  ...
  xml(:resource) do
    context 'buyer account' do
      ...
      # it { should have_tags('admin_user_display_name').from(resource) }
      ...
      #
      # context 'if scheduled_for_deletion' do
      #   let(:resource) { FactoryBot.build(:provider_account, state: 'scheduled_for_deletion', state_changed_at: Time.zone.now).decorate }
    ...
    context 'provider account' do
      let(:resource) do
        provider = FactoryBot.create(:simple_provider, support_email: 'support@email.com', finance_support_email: 'finance@email.com', site_access_code: 'access-code')
