...
import { EditProductPage } from 'components/pages/product'

jest.mock('react-async')

const product: IProduct = factories.Product.build()

const setup = (asyncState: Partial<AsyncState<IProduct>>) => {
  (useAsync as jest.Mock).mockReturnValueOnce(asyncState)
  return render(<EditProductPage productId={String(product.id)} />)
}

it('should render properly', () => {
  const { getByText } = setup({ data: product })

  expect(getByText('pepe')).toBeInTheDocument()
})
