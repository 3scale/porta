...
import { ActionHandlers, Action, createReducer } from 'utils'
...
const useDataListTable = (initialState: TableState) => {
  const [state, setState] = useState(initialState)
  const dispatch = (action: Action<any>) => setState(tableReducer(state, action))

  return {
    columns: state.columns,
    rows: state.rows,
    selectedRows: state.rows.filter((r) => Boolean(r.selected)),
    sortBy: state.sortBy,
