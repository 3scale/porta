...
class UserTest < ActiveSupport::TestCase
  ...
  test 'before destroying user of first_admin_id updates its attribute to the next one' do
    provider = FactoryGirl.create(:simple_provider)
    FactoryGirl.create(:member, account: provider)
    admin_users = FactoryGirl.create_list(:admin, 2, account: provider)

    user_id = provider.first_admin_id
    User.find(user_id).destroy!
    provider.reload
    assert_not_equal user_id, provider.first_admin_id
    assert_equal admin_users.last.id, provider.first_admin_id
  end

  test 'before destroying user of first_admin_id it keeps the same attribute if there is no next admin' do
    provider = FactoryGirl.create(:simple_provider)
    FactoryGirl.create(:member, account: provider)
    admin_user = FactoryGirl.create(:admin, account: provider)

    user_id = provider.first_admin_id
    admin_user.stubs(destroyable?: true)
    admin_user.destroy!
    provider.reload
    assert_equal user_id, provider.first_admin_id
  end

  test 'before destroying user of first_admin_id it keeps the same attribute if the only admin left is the impersonation admin' do
    provider = FactoryGirl.create(:simple_provider)
    FactoryGirl.create(:member, account: provider)
    admin_user = FactoryGirl.create(:admin, account: provider)
    FactoryGirl.create(:admin, account: provider, username: ThreeScale.config.impersonation_admin['username'])
  ...
  test 'before destroying user of first_admin_id, there are no errors raised by the callback if the account is already deleted' do
    provider = FactoryGirl.create(:simple_provider)
  ...
  test 'after creating the first admin user, the account first_admin_id is correctly set' do
    provider = FactoryGirl.create(:simple_provider)
    user = FactoryGirl.create(:admin, account: provider)

    assert_equal user.id, provider.first_admin_id
  end

  test 'after creating a member, the impersonation admin, and 2 regular admin users, the 1st regular admin user is set as the account first_admin_id' do
    provider = FactoryGirl.create(:simple_provider)
    FactoryGirl.create(:member, account: provider)
    FactoryGirl.create(:admin, account: provider, username: ThreeScale.config.impersonation_admin['username'])
    admin_users = FactoryGirl.create_list(:admin, 2, account: provider)

    assert_equal admin_users.first.id, provider.first_admin_id
  end
