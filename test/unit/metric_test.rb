...
class MetricTest < ActiveSupport::TestCase
  ...
  test '#destroyable?' do
    ...
    assert metric = service.metrics.hits
  ...
  test 'index uniq of system_name in service scope' do
    ...
    assert_raise ActiveRecord::RecordNotUnique do
      metric_two.update_column(:system_name, 'frags') # rubocop:disable Rails/SkipsModelValidations
    end
  end
  ...
  test 'index uniq of system_name in owner scope' do
    ...
    owners.each do |owner|
      ...
      FactoryBot.create(:metric, **owner_attributes, system_name: 'frags')
  ...
  test 'system_name is not case sensitive' do
    ...
    assert metric_two.update(system_name: 'Frags')
  ...
  test 'return only top-level metric on top_level' do
    ...
    metric_two = FactoryBot.create(:metric, service: service)
    FactoryBot.create(:metric, parent: service.metrics.hits, service: service)
  ...
  test '.hits returns first metric if there is no one called hits' do
    ...
    FactoryBot.create(:metric, system_name: 'bars', service: service)
...
class ChildMetricTest < ActiveSupport::TestCase
  def setup
    ...
    @child_metric = @parent_metric.children.create!(system_name: 'articles/create', friendly_name: 'Create an article')
  end

  test 'should respond true to child?' do
    assert @child_metric.child?
  end

  test 'should have the same unit as the parent metric' do
    assert_equal @parent_metric.unit, @child_metric.unit
  end

  test 'should have readonly unit' do
    @child_metric.unit = 'call'
    assert_equal 'hit', @child_metric.unit
  end

  test 'should be associated to the same service as parent metric' do
    assert_equal @service, @child_metric.service
  end

  test 'should only be child of hits' do
...
class VisibilityOptionsForPlanTest < ActiveSupport::TestCase
  ...
  test 'should be visible by default' do
  ...
  test 'should be limits_only_text by default' do
    assert @metric.limits_only_text_in_plan?(@plan)
  end

  test 'should toggle limits_only_text' do
    assert @metric.limits_only_text_in_plan?(@plan)
...
class AvailabilityForPlanTest < ActiveSupport::TestCase
  ...
  test 'should be enabled for plan if no limit 0 is present' do
  ...
  test 'should be disabled for plan if a limit 0 is present' do
  ...
  test 'should disable only the given metric' do
    assert @metric1.enabled_for_plan?(@plan)
  ...
  test 'should disable the given metric even if usage limit already exists' do
  ...
  test 'should not disable the given metric - all periods are being used' do
  ...
  test 'should enable only the given metric' do
    assert @metric1.disabled_for_plan?(@disabled_plan)
