...
class ApiDocs::ProviderUserDataTest < ActiveSupport::TestCase
  ...
  def test_service_tokens
    ...
    member = FactoryBot.create(:simple_user, account: @admin.account, allowed_sections: ['plans'], allowed_service_ids: [service.id])
  ...
  class MemberPermissions < ActiveSupport::TestCase
    ...
    test 'member users can only see applications of authorized services' do
      member = FactoryBot.create(:simple_user, account: @admin.account, allowed_sections: ['plans'], allowed_service_ids: [@authorized_service.id])
