...
class ApiDocs::ProviderUserDataTest < ActiveSupport::TestCase
  ...
  class MemberPermissions < ActiveSupport::TestCase
    def setup
      ...
      @authorized_service = FactoryGirl.create(:simple_service, account: @admin.account)
      @unauthorized_service = FactoryGirl.create(:simple_service, account: @admin.account)

      authorized_application_plan = FactoryGirl.create(:application_plan, issuer: @authorized_service)
      unauthorized_application_plan = FactoryGirl.create(:application_plan, issuer: @unauthorized_service)

      @authorized_cinstance = FactoryGirl.create(:simple_cinstance, plan: authorized_application_plan, state: 'live')
      @unauthorized_cinstance = FactoryGirl.create(:simple_cinstance, plan: unauthorized_application_plan, state: 'live')
    end
    ...
    test 'member users can only see applications of authorized services' do
      member = FactoryGirl.create(:simple_user, account: @admin.account)
      FactoryGirl.create(:member_permission, user: member, admin_section: :services, service_ids: [@authorized_service.id])
    ...
    test 'member users see no application if no service id is authorized' do
      member = FactoryGirl.create(:simple_user, account: @admin.account)
      FactoryGirl.create(:member_permission, user: member, admin_section: :services, service_ids: [])
