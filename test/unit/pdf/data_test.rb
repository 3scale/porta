...
class Pdf::DataTest < ActiveSupport::TestCase
  ...
  test 'scope data by service' do
    service_two = Factory(:service, :account => @provider_account)
    plan = Factory( :application_plan, :issuer => service_two)
    cinstance = Factory(:cinstance, :plan => plan)

    service_three = Factory(:service, :account => @provider_account)
  ...
  test 'return distinct accounts' do
    buyer      = Factory(:buyer_account, :provider_account => @provider_account)
    plan       = Factory(:application_plan, :issuer => @service)
    cinstance  = Factory(:cinstance, :plan => @plan, :user_account => buyer)
    cinstance2 = Factory(:cinstance, :plan => plan, :user_account => buyer)
    cinstance3 = Factory(:cinstance, :user_account => buyer)
  ...
  test 'return ordered accounts' do
    buyer1     = Factory(:buyer_account, :org_name => 'first', :provider_account => @provider_account, :created_at => 1.day.ago)
    buyer2     = Factory(:buyer_account, :org_name => 'second', :provider_account => @provider_account, :created_at => 5.days.ago)
    buyer3     = Factory(:buyer_account, :org_name => 'third', :provider_account => @provider_account, :created_at => 10.days.ago)

    plan1 = Factory( :application_plan, :issuer => @service, :created_at => 10.days.ago)
    plan2 = Factory( :application_plan, :issuer => @service, :created_at => 5.days.ago)
    plan3 = Factory( :application_plan, :issuer => @service, :created_at => 1.day.ago)

    cinstance1 = Factory(:cinstance, :plan => plan1, :user_account => buyer1)
    cinstance2 = Factory(:cinstance, :plan => plan2, :user_account => buyer2)
    cinstance3 = Factory(:cinstance, :plan => plan3, :user_account => buyer3)
