...
class OAuthFlowPresenterTest < ActiveSupport::TestCase
  ...
  def test_sso_integration_callback_url
    ...
    presenter = OauthFlowPresenter.new(@authentication_provider, @request)
    assert_equal "http://#{@provider.internal_domain}/auth/#{@authentication_provider.system_name}/callback", presenter.sso_integration_callback_url

    @authentication_provider.kind = 'auth0'
    presenter = OauthFlowPresenter.new(@authentication_provider, @request)
    assert_equal "http://#{@provider.internal_domain}/auth/#{@authentication_provider.system_name}/callback, " \
      "http://#{@provider.internal_domain}/auth/invitations/auth0/#{@authentication_provider.system_name}/callback",
        presenter.sso_integration_callback_url
  end

  test 'callback_endpoint' do
    expected_url = "http://#{@provider.internal_domain}/auth/#{@authentication_provider.system_name}/callback?plan_id=42"
    assert_equal expected_url, @presenter.callback_url
  end

  test 'callback_endpoint with query overriden' do
    expected_url = "http://#{@provider.internal_domain}/auth/#{@authentication_provider.system_name}/callback?code=secret"
  ...
  test 'master callback endpoint' do
    ...
    expected_url = "http://#{Account.master.internal_domain}/master/devportal/auth/#{authentication_provider.system_name}/callback?domain=#{authentication_provider.account.internal_domain}"
    assert_equal expected_url, presenter.callback_url
  end
