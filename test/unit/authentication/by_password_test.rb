...
class Authentication::ByPasswordTest < ActiveSupport::TestCase
  ...
  class UnicodeNormalizationTest < Authentication::ByPasswordTest
    test 'password is normalized to NFC before saving' do
      ...
      assert user.authenticate(password_nfc), 'Should authenticate with NFC form'
      assert user.authenticate(password_nfd), 'Should authenticate with NFD form (normalized before comparison)'
    end
