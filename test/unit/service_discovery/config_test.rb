...
class ServiceDiscovery::ConfigAndTest < ActiveSupport::TestCase
  def subject
    ThreeScale.config.merge!(service_discovery: config)
  ...
  test 'authentication_method' do
    ...
    config.merge!(authentication_method: nil)
  ...
  test 'oauth_server_type' do
    ...
    config.merge!(oauth_server_type: nil)
