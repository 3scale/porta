...
class SuspendInactiveAccountsWorkerTest < ActiveSupport::TestCase
  setup do
    ...
    old_tenant_with_old_traffic.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
    FactoryBot.create(:cinstance, user_account: old_tenant_with_old_traffic, first_daily_traffic_at: MaxAllowedDaysLoader.load_account_inactivity.ago)
    @accounts[:to_suspend] << old_tenant_with_old_traffic

    old_buyer_with_old_traffic = FactoryBot.create(:simple_buyer)
    old_buyer_with_old_traffic.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
    FactoryBot.create(:cinstance, user_account: old_buyer_with_old_traffic, first_daily_traffic_at: MaxAllowedDaysLoader.load_account_inactivity.ago)
    @accounts[:not_to_suspend] << old_buyer_with_old_traffic

    recent_tenant_without_traffic = FactoryBot.create(:simple_provider)
    recent_tenant_without_traffic.update_attribute(:created_at, (MaxAllowedDaysLoader.load_account_inactivity - 1.day).ago)
    @accounts[:not_to_suspend] << recent_tenant_without_traffic

    recent_tenant_with_recent_traffic = FactoryBot.create(:simple_provider)
    recent_tenant_with_recent_traffic.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
    FactoryBot.create(:cinstance, user_account: recent_tenant_with_recent_traffic, first_daily_traffic_at: (MaxAllowedDaysLoader.load_account_inactivity - 1.day).ago)
    @accounts[:not_to_suspend] << recent_tenant_with_recent_traffic

    old_tenant_without_traffic = FactoryBot.create(:simple_provider)
    old_tenant_without_traffic.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
    @accounts[:to_suspend] << old_tenant_without_traffic

    master_account.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
  ...
  test 'it does not perform for already suspended or deleted accounts' do
    ...
    already_suspended.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
    already_deleted = FactoryBot.create(:simple_provider, state: 'scheduled_for_deletion', state_changed_at: old_time)
    already_deleted.update_attribute(:created_at, MaxAllowedDaysLoader.load_account_inactivity.ago)
