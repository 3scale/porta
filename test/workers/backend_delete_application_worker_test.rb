...
class BackendDeleteApplicationWorkerTest < ActiveSupport::TestCase
  ...
  test 'perform' do
    ...
    perform_enqueued_jobs { BackendDeleteApplicationWorker.perform_later(event.event_id) }
  end
  ...
  test 'perform reports error when the event does not exist' do
    ...
    perform_enqueued_jobs { BackendDeleteApplicationWorker.perform_later('fake-id') }
  end
end
