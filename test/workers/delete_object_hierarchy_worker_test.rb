...
class DeleteObjectHierarchyWorkerTest < ActiveSupport::TestCase
  ...
  class ReschedulingTest < ActiveSupport::TestCase
    ...
    test "will reschedule if timeout is reached" do
      ...
      # DeleteObjectHierarchyWorker.any_instance.expects(:now).returns(now + DeleteObjectHierarchyWorker::WORK_TIME_LIMIT_SECONDS)
      # DeleteObjectHierarchyWorker.any_instance.stubs(:now).returns(now)
  ...
  class StaleObjectErrorTest < ActiveSupport::TestCase
    ...
    setup do
      BlockingLoadTarget.queue_to_resume.clear
      BlockingLoadTarget.queue_for_blocked.clear
      BlockingLoadTarget.test_target_object = nil
    end

    teardown do
      BlockingLoadTarget.queue_for_blocked.close
      BlockingLoadTarget.queue_to_resume.close
      BlockingLoadTarget.test_target_object = nil
    end

    module BlockingLoadTarget
      class << self
        def queue_to_resume
          @queue_to_resume ||= Queue.new
        end

        def queue_for_blocked
          @queue_for_blocked ||= Queue.new
        end

        attr_accessor :test_target_object
      end

      # Overriding the `delete` method of HasOneAssociation https://github.com/rails/rails/blob/v7.0.8.7/activerecord/lib/active_record/associations/has_one_association.rb#L26-L51
      # When entering this method, override the `load_target` method so we can hand over the execution to the main thread
      def load_target_blocking
        load_target_original.tap do |target|
          puts "I'm alive, caller: #{caller[2]}"
          next unless target && target == BlockingLoadTarget.test_target_object && caller[2].end_with?("`delete'")
          puts "will be waiting now"
          BlockingLoadTarget.queue_for_blocked.push "I'm wating now."
          puts "waiting"
          BlockingLoadTarget.queue_to_resume.pop
          puts "continuing"
        end
      end
    end
    ...
    def test_race_condition
      Service.stubs(:background_deletion).returns([]) # we want background worker to just destroy without separating children

      service = FactoryBot.create(:simple_service)
      # There is a restriction on deleting service, at least one should remain so here we add another one
      ...
      unless ActiveRecord::Associations::HasOneAssociation.method_defined?(:load_target_blocking)
        ActiveRecord::Associations::HasOneAssociation.alias_method(:load_target_original, :load_target) unless ActiveRecord::Associations::HasOneAssociation.method_defined?(:delete_original)
        ActiveRecord::Associations::HasOneAssociation.prepend BlockingLoadTarget
      end
      ActiveRecord::Associations::HasOneAssociation.alias_method :load_target, :load_target_blocking
      BlockingLoadTarget.test_target_object = proxy

      # Destroy the proxy in another thread but at the right time
      proxy_delete_thr = Thread.new do
        puts "waiting to delete"
        BlockingLoadTarget.queue_for_blocked.pop
        puts "deleting"
        proxy.destroy!
        puts "deleted"
      ensure
        BlockingLoadTarget.queue_to_resume.push "Proxy deleted, you can continue now, fella." rescue nil
      end

      # Execute deletion of service but suspend the execution of deleting the proxy by `:dependent => :destroy`
      assert_raise(ActiveRecord::StaleObjectError) do
        perform_enqueued_jobs(queue: :deletion) { DeletePlainObjectWorker.delete_later(service) }
      end

      assert proxy_delete_thr.join(5), "sneaky thread did not exit, must be some logic error"
      # assert_raise(ActiveRecord::RecordNotFound) { proxy.reload }
      # assert_raise(ActiveRecord::RecordNotFound) { service.reload }
    ensure
      if proxy_delete_thr
        ActiveRecord::Associations::HasOneAssociation.alias_method :load_target, :load_target_original
      end
      proxy_delete_thr&.kill
    end
  end
end
