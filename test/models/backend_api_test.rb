...
class BackendApiTest < ActiveSupport::TestCase
  def setup
    ...
    @backend_api = BackendApi.new(account: @account, name: 'My Backend', private_endpoint: 'http://example.com')
  end
  ...
  test 'proxy api backend with base path' do
    ...
    @backend_api.valid?
    assert_equal [@backend_api.errors.generate_message(:private_endpoint, :invalid_url)], @backend_api.errors.messages[:private_endpoint]
