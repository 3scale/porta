...
class SecureHeadersTest < ActionDispatch::IntegrationTest
  ...
  test 'adds secure headers in the response' do
    ...
    assert_equal "default-src 'self'; font-src 'self' ; img-src 'self'  data:; script-src 'self'  'unsafe-inline' 'unsafe-eval'; style-src 'self'  'unsafe-inline'; connect-src 'self' http://localhost:3001", response.headers['Content-Security-Policy']
  end
