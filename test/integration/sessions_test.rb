...
class SessionsTest < ActionDispatch::IntegrationTest
  ...
  test 'redirect_url parameter is discarded if login is not via sso' do
    user = FactoryBot.create(:user, account: @buyer, username: 'xi@example.net', password: 'Supersecret123+!')
    user.activate

    host! @provider.internal_domain

    get developer_portal.create_session_url(username: user.username, password: 'Supersecret123+!', redirect_url: forum_url(host: @provider.internal_domain))
    follow_redirect!
  ...
  test 'passing redirect_to to login form' do
    ...
    post developer_portal.session_path(username: 'xi@example.net', password: 'Supersecret123+!')
  ...
  test 'current user is not persisted across domains' do
    provider_user = FactoryBot.create(:user, account: @provider, username: 'provider', password: 'Supersecret123+!')
    provider_user.activate!

    open_session do |session|
      session.host! @provider.external_admin_domain
      session.post provider_sessions_path(username: 'provider', password: 'Supersecret123+!')
  ...
  test 'mixpanel event properties are not persisted across requests' do
    ...
    other_provider_user = FactoryBot.create(:user, account: @provider, username: 'other_provider', password: 'Supersecret123+!')
    ...
    open_session do |session|
      ...
      session.host! @provider.external_admin_domain
      session.post provider_sessions_path(username: 'other_provider', password: 'Supersecret123+!')
