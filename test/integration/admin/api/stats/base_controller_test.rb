...
class Stats::Data::BaseControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'required params' do
    ...
    get usage_admin_api_stats_services_path(url_params), stats_params
    assert_response :success

    get usage_admin_api_stats_services_path(url_params), stats_params.except(:metric_name)
    assert_response :bad_request
    assert_equal 'Required parameter missing: metric_name', response.body

    get usage_admin_api_stats_services_path(url_params), stats_params.except(:period)
    assert_response :bad_request
    assert_equal 'Required parameter missing: granularity', response.body

    get usage_admin_api_stats_services_path(url_params), stats_params.except(:period).merge(granularity: 'hour')
    assert_response :bad_request
    assert_equal 'Required parameter missing: since', response.body

    get usage_admin_api_stats_services_path(url_params), stats_params.except(:period).merge(granularity: 'hour', since: '')
    assert_response :bad_request
    assert_equal 'Required parameter missing: since', response.body

    get usage_admin_api_stats_services_path(url_params), stats_params.except(:period).merge(granularity: 'hour', since: Time.utc(2020, 3, 19).to_date, until: Time.utc(2020, 3, 21).to_date)
    assert_response :success
