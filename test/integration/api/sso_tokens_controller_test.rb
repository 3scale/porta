...
class Admin::Api::SsoTokensControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'successful post create' do
    post admin_api_sso_tokens_path(access_token: @access_token), params: { sso_token: { username: 'alaska123', expires_in: 60 } }
    assert_response :success

    # provider key
    post admin_api_sso_tokens_path(provider_key: @admin.account.api_key), params: { sso_token: { username: 'alaska123', expires_in: 60 } }
    assert_response :success
  end
  ...
  class Admin::Api::SsoTokensControllerForProviderTest < ActionDispatch::IntegrationTest
    ...
    def setup
      ...
      host! @admin.account.internal_admin_domain
    end
