class DeveloperPortal::Admin::Applications::AlertsController <  ::DeveloperPortal::BaseController
  ...
  def collection
    current_account.alerts.not_deleted.by_application(@cinstance).sorted.order_by(alert_params[:sort], alert_params[:direction])
  end

  def resource
    collection.find(alert_params[:id])
  end

  def alert_params
    params.permit(%i[sort direction id]).to_h
  end
