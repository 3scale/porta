...
namespace :multitenant do
  namespace :tenants do
    ...
    task suspend_forbidden_plans_scheduled_for_deletion: :environment do
      return unless Features::AccountDeletionConfig.enabled?
      ...
      query = Account.tenants.scheduled_for_deletion.where.has do
        exists Cinstance.by_account(BabySqueel[:accounts].id).with_application_plans_with_system_names(forbidden_plans_to_be_auto_destroyed).select(:id)
      end
